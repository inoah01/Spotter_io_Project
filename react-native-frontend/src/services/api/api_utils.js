...
//  - Check backend authentication function and ensure relevant, user-friendly messages are returned
...
export const userLogin = async (payload) => {
  ...
  try {
    ...
    if (error.response) {
      ...
      errorMessage = "Server unresponsive. Please try again later.";
    } else {
...
export const createUser = async (signupInfoWithUid) => {
  const headers = unprotectedHeaders;

  if (!signupInfoWithUid) {
    console.log("Data not received from handleCreateAccount");
    return;
  }

  try {
    const response = await axiosClient.post("/users/create", signupInfoWithUid, {
